#!/usr/bin/env node
const koch_antisnowflake = require('./index.js');

const printUsage = function(showIntro) {
    if (showIntro) {
        console.log(koch_antisnowflake.create(2));
        console.log(' Print the Koch Anti-Snowflake to the console!');
    }
    console.log('\n' + 
                ' Usage:\n' + 
                ...
                '\n' +
                ' Options:\n' + 
                '   --rotate=<rotate>        Rotate the Koch Anti-Snowflake: [flip|standard]\n' +
...
if (process.argv.length > 2) {
    ...
    if (values[0] && !isNaN(values[0]) && parseInt(values[0]) >= 0) {
        ...
        if (n !== undefined && s !== undefined) {
            console.log(koch_antisnowflake.create(n, { size: s, rotate: getRotation(flags), character: getCharacter(flags) } ));
        }
    } else {
